...
export const CUTLINE_OUTER_PATH = "M0 0 H590 V96 L574 112 V691 L590 707 V1165 L575 1180 H0 Z";
...
export function createCutShapeMask(widthOrOptions = CUTLINE_WIDTH, height = CUTLINE_HEIGHT,) {
  ...
  for (let y = 0; y < dimensions.height; y += 1) {
    ...
    for (let x = 0; x < dimensions.width; x += 1) {
      ...
      const inHole = isInCircle(sampleX, sampleY, CUTLINE_HOLES.leftCircle) || isInCapsule(sampleX, sampleY) || isInCircle(sampleX, sampleY, CUTLINE_HOLES.rightCircle);
      if (!inHole) mask[rowOffset + x] = 255;
    }
  }
  return dimensions.mirror ? mirrorMaskHorizontally(mask, dimensions.width, dimensions.height) : mask;
}
...
export function createCutlineSvg(options = {}) {
  const {
    width = CUTLINE_WIDTH, height = CUTLINE_HEIGHT, stroke = "#ff00ff", strokeWidth = 1, mirror = false,
  } = options;
  ...
  const attrs = ['xmlns="http://www.w3.org/2000/svg"', `width="${escapeXmlAttribute(width)}"`, `height="${escapeXmlAttribute(height)}"`, `viewBox="0 0 ${CUTLINE_WIDTH} ${CUTLINE_HEIGHT}"`, 'fill="none"', `stroke="${escapeXmlAttribute(stroke)}"`, `stroke-width="${escapeXmlAttribute(strokeWidth)}"`, 'stroke-linejoin="round"', 'vector-effect="non-scaling-stroke"',].join(" ");

  return [`<svg ${attrs}>`, '  <title>CutContour</title>', `  <g id="CutContour" data-spot-color="CutContour"${mirror ? ` transform="translate(${CUTLINE_WIDTH} 0) scale(-1 1)"` : ""}>`, `    <path d="${CUTLINE_OUTER_PATH}"/>`, '    <circle cx="42" cy="42" r="18"/>', '    <rect x="160" y="24" width="272" height="36" rx="18"/>', '    <circle cx="550" cy="42" r="18"/>', "  </g>", "</svg>",].join("\n");
}
...
function requireContextMethods(context) {
  const required = ["beginPath", "moveTo", "lineTo", "closePath", "arc", "stroke",];
...
export function drawCutlineCanvas(target, options = {}) {
  ...
  const {
    stroke = "#ff00ff", lineWidth = 1, lineJoin = "round", lineCap = "round", mirror = false,
  } = options;
