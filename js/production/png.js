...
function readPngChunks(value) {
  ...
  if (bytes.byteLength < PNG_SIGNATURE.byteLength) {
    throw new TypeError("The value is too short to be a PNG image.");
  }

  for (let index = 0; index < PNG_SIGNATURE.byteLength; index += 1) {
    if (bytes[index] !== PNG_SIGNATURE[index]) {
      throw new TypeError("The value does not contain a valid PNG signature.");
    }
  }
  ...
  while (offset < bytes.byteLength) {
    ...
    if (!Number.isSafeInteger(endOffset) || endOffset > bytes.byteLength) {
      throw new TypeError("The PNG contains a truncated chunk.");
    }
    ...
    chunks.push({
      ...
      length,
      offset,
      type: chunkType(bytes, offset + 4),
    });
    offset = endOffset;
  }

  if (chunks.length === 0 || chunks[0].type !== "IHDR" || chunks[0].length !== 13) {
    throw new TypeError("The PNG must start with a 13-byte IHDR chunk.");
  }

  if (chunks.at(-1)?.type !== "IEND") {
    throw new TypeError("The PNG must end with an IEND chunk.");
  }
...
export function canvasToPngBlob(canvas) {
  if (canvas == null || typeof canvas.toBlob !== "function") {
    return Promise.reject(new TypeError("The canvas must provide a toBlob() method."));
  }
...
/**
 * Reads the first pHYs chunk without mutating the supplied PNG bytes.
 * `count` reports how many pHYs chunks are present, which is useful when
 * verifying that duplicate metadata was removed.
 *
...
export function parsePngPhys(value) {
  ...
  if (physChunks.length === 0) {
    return null;
  }

  const first = physChunks[0];
  if (first.length !== 9) {
    throw new TypeError("A PNG pHYs chunk must contain exactly 9 data bytes.");
  }
...
/**
 * Returns a PNG Blob with exactly one pHYs chunk for the requested DPI.
 * Existing pHYs chunks are removed and the new chunk is placed after IHDR.
 *
 * @param {Blob} blob
 * @param {number} dpi
 * @returns {Promise<Blob>}
...
export async function setPngDpi(blob, dpi = 300) {
  if (blob == null || typeof blob.arrayBuffer !== "function") {
    throw new TypeError("Expected a Blob containing PNG data.");
  }

  if (typeof dpi !== "number" || !Number.isFinite(dpi) || dpi <= 0) {
    throw new RangeError("DPI must be a finite number greater than zero.");
  }

  const pixelsPerMetre = Math.round(dpi / METRES_PER_INCH);
  if (pixelsPerMetre < 1 || pixelsPerMetre > 0xffffffff) {
    throw new RangeError("DPI is outside the range supported by PNG pHYs metadata.");
  }
  ...
  const outputLength = PNG_SIGNATURE.byteLength
    + physChunk.byteLength
    + keptChunks.reduce((total, chunk) => total + (chunk.endOffset - chunk.offset), 0);
...
export function downloadBlob(blob, filename) {
  if (typeof document === "undefined" || document.body == null) {
    throw new Error("Blob downloads require a browser document with a body.");
  }
  ...
  if (
    ...
    throw new Error("Blob downloads require URL.createObjectURL().");
  }

  if (typeof filename !== "string" || filename.trim() === "") {
    throw new TypeError("A non-empty download filename is required.");
  }
  ...
  try {
    ...
    // Keep the URL alive long enough for browsers to start reading it. Some
    // engines cancel a programmatic download when it is revoked in the same
    // task as anchor.click().
    globalThis.setTimeout(() => urlApi.revokeObjectURL(objectUrl), 1000);
  }
}

/**
 * Encodes a canvas, writes PNG DPI metadata, and downloads the result.
 *
 * @param {HTMLCanvasElement|OffscreenCanvas} canvas
 * @param {string} filename
 * @param {number} dpi
 * @returns {Promise<Blob>} the final PNG Blob
 */
